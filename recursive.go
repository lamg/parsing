...
func Parse(grammar []Symbol, tks TokenStream) (t *Tree, e error) {
	tks.Next()
	t, e = parse(grammar, 2, tks)
	if e == nil {
		tk, err := tks.Current()
		if err == nil {
...
func parse(grammar []Symbol, curr int, tks TokenStream) (t *Tree, e error) {
	parsed, match := []*Tree{}, false
	for curr != 0 {
		var nt *Tree
		if grammar[curr].IsTerminal {
			nt, e = matchTk(tks, &grammar[curr])
		} else {
			nt, e = parse(grammar, grammar[curr].Header, tks)
		}
		match = nt != nil
		if match {
			if nt.Token != nil {
			...
			curr = grammar[curr].Next
		} else {
			curr = grammar[curr].Alt
		}
	}
	if match {
		t = build(parsed)
